  ...
  data() {
    return {
      ...
      dataRule: {
        ...
        skuNum: [{ required: true, message: "购买个数不能为空", trigger: "blur" }],
        taskId: [{ required: true, message: "工作单id不能为空", trigger: "blur" }],
      },
    };
  },
  methods: {
        ...
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(
              `/ware/wareordertaskdetail/${!this.dataForm.id ? "save" : "update"}`
            ),
            method: "post",
